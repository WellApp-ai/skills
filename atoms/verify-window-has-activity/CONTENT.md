...
None → stop. A window with no transactions produces a figure of zero, and zero standing on nothing measured is not a reading{{#if purpose}} — say that instead of reporting it{{/if}}. Two situations look identical here and must not be described identically: a feed that has delivered nothing yet, and a real month in which nothing moved. The freshness step above tells them apart, so cite what it found rather than guessing.
