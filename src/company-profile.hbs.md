...
1. **Pin the workspace.** {{> define-workspace purpose="to compose a full profile of one company"}}

2. **Confirm the connections this answer needs.** {{> connect-tools purpose="to compose a full profile of one company" kinds="invoicing, accounting" internalCheck=true}}
...
6. **Frame the relationship.** {{> confirm-my-company purpose="to frame whether this company is a customer or a vendor" consequence="inverts customer and vendor" foldAliases=true foldCounterparties=true onDecline="report the invoice relationship without labeling either side"}} Use its `identity_set` as "us" and its `counterparty_alias_sets` when gathering the profiled company's invoices.
...
7. **Normalize currency.** If invoice totals span more than one currency, {{> normalize-currency}} Report its `converted_total` with those rates, or its `per_currency` breakdown — never a blended total. Build any per-row figure from its `converted` entries, matched back by tag, rather than re-applying rates yourself.
