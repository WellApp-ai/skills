...
| `fetch-missing-invoices` | "Fetch the invoices I'm missing" — the whole month-end sweep in one prompt: workspace, month, bank when a month holds no bank transaction, the gap list, then queues the invoice-fetching agents on your Deploy click. | [View details →](docs/fetch-missing-invoices.md) |
...
| `assign-missing-invoices` | "Assign these expenses to Marie and Théo" — the expense lines still missing an invoice for a month, split into no owner set, yours, and owned by others, each assignable to a set of people from the card. | [View details →](docs/assign-missing-invoices.md) |
| `deploy-agents` | "Go get those invoices" — previews which invoice-fetching agents Well would run per provider, then on your Deploy click queues the fetch tasks and hands you the collect link that starts them. | [View details →](docs/deploy-agents.md) |
...
| `assign-missing-invoices` | The expense lines missing an invoice for a month, split into no owner set, yours, and owned by others, each assignable to a set of people from the card. | [⬇ Download for Claude Desktop](https://github.com/WellApp-ai/skills/raw/main/dist/assign-missing-invoices.skill) · [.zip](https://github.com/WellApp-ai/skills/raw/main/dist/assign-missing-invoices.zip) |
| `deploy-agents` | Preview the invoice-fetching agents Well would run for a period, then queue the fetch tasks on your Deploy click and get the collect link that starts them. | [⬇ Download for Claude Desktop](https://github.com/WellApp-ai/skills/raw/main/dist/deploy-agents.skill) · [.zip](https://github.com/WellApp-ai/skills/raw/main/dist/deploy-agents.zip) |
