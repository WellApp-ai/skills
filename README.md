...
| `fetch-missing-invoices` | "Fetch the invoices I'm missing" — the whole month-end sweep in one prompt: workspace, month, bank when the month holds no bank transaction, the gap list, then a preview of the agents that would fetch them. | [View details →](docs/fetch-missing-invoices.md) |
