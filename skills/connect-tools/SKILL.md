...
4. **State the coverage once, then end the turn on the card.** Say one line per requested kind — what is connected, what is missing or in error, and why it matters for the job (`purpose`) — then one closing line: tick what is missing on the card and click Connect if you want, then click Continue. Do not restate the rows or re-render them as a list or table: the card carries the tiles, the install links, its own scope title and the workspace strip. Even when every kind is green, the card stays on screen for the user to see and confirm the state, and the turn still ends here. Nothing else in the turn.
