...
3. **Read the current coverage in one call.** `well_list_connectors({ workspace_id, from_selection: true })` when this run follows a vendor pick, `well_list_connectors({ workspace_id, kind })` when the job covers exactly one kind, `well_list_connectors({ workspace_id })` otherwise. When the caller supplied `title` / `subtitle`, add them to whichever shape applies — `well_list_connectors({ workspace_id, kind, title, subtitle })` — so the card renders the flow's own close-context copy over the scope-derived default; omit them entirely when the caller passed none, and the card keeps its scope wording. The result renders the connect picker card. On a `kind`-scoped or unscoped call, keep the `direction: input` rows, group them by `data_domains`, and read each row's state with the precedence in Tooling. Per requested kind:
