...
**Entering this step means rendering its card.** Whatever asks for a connect step — a user, a calling flow, or a missing-invoices pick that names connectors Well already carries — the step is `well_list_connectors` plus the card its result renders, and the turn ends there. **Describing the connectable rows in prose and moving to the next step is the failure this rule names**, not a shorter path to the same place: the install link lives on the card and nowhere else, so a reader given only a sentence has nothing to click and the step never happened. Render the card, or say the step did not run.

**One card, three scopes, and the result names which one.** `well_list_connectors` takes `kind` (one financial domain), `q` (one name search), or `from_selection: true` (the connectors behind the counterparties the user picked on the missing-invoices card this session, every one pre-checked). They are mutually exclusive. Every result reports `scope` — `"catalog"`, one of the three domains, or `"picked_vendors"` — and the card titles itself from that field, so its wording always describes the rows it is showing rather than the call that asked for them.
...
- The step rendered its card. No run described the connectable rows in prose and moved on without one.
- The scope matched the job: `from_selection: true` where the user had already picked the vendors, `kind` where the choice was still open, and never both. The coverage line described the rows the result's `scope` names, never a domain the card was not listing.
