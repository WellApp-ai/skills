...
4. **State the coverage once. In `flow_step` mode, end the turn on the card.** Say one line per requested kind — what is connected, what is missing or in error, and why it matters for the job (`purpose`). Do not restate the rows or re-render them as a list or table: the card carries the tiles and the install links.
