...
**When `install_all_url` is non-null it is the only install link you offer.** Hand the user that one link. Do not list the per-connector `install_url` links beside it — a table of links puts the reader through a sign-in per tool for one decision they already made. Name the `install_all_omitted` rows with their own `install_url`, because the batch link does not reach them. When `install_all_url` is null, the rows' own `install_url` links are the offer — list them, and never announce a batch link the result does not carry. **A reconnect is one connector, not a batch**: repairing a named errored connection is always that row's own `install_url`. The batch link installs what is missing. You pass through the links the tool returned, and never build or edit one yourself.
...
4. **State the coverage once. In `flow_step` mode, end the turn on the card.** Say one line per requested kind — what is connected, what is missing or in error, and why it matters for the job (`purpose`). Do not restate the rows or re-render them as a list or table: the card carries the tiles, the install links, its own scope title and the workspace strip.
   ...
   - In a text-only host, name at most three connectors per missing kind — `is_preselected` rows first, then the user's provider hints via `q` — and treat the user's next message as the answer. Give one link, not one per tool. When the result carries `install_all_url`, that link is the whole offer: it installs the result's connectors in a single tab, so the user signs in once. Do not print the rows' own `install_url` links beside it. Name the `install_all_omitted` rows with their own `install_url`, because the batch link does not reach them. When `install_all_url` is null, the named rows' own `install_url` links are the offer.
...
- A per-connector list of `install_url` links where the result carried `install_all_url` — one link is the offer, and a list of them costs the user a sign-in per tool.
...
A text-only host — nothing renders a card. The user asks to connect their accounting tool, so the call is `well_list_connectors({ workspace_id, kind: "accounting" })`. The result comes back with `install_all_url` set and `install_all_omitted` empty.

### Expected behavior

Name at most three connectors — the `is_preselected` rows first — then give the one `install_all_url` link and nothing else. It installs them in a single tab, so the user signs in once. Do not print each row's `install_url` beside it. Had `install_all_omitted` named a row, name that connector separately with its own `install_url`, because the batch link does not reach it. Had `install_all_url` come back null — an unscoped read, or the whole bank domain — the named rows' own `install_url` links would be the offer, and no batch link would be mentioned at all.
