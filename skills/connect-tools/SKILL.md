...
3. **Read the current coverage in one pass.** Call `well_list_connectors({ workspace_id })` for all three kinds, or `well_list_connectors({ workspace_id, kind })` once per requested kind when the caller scoped the job. Keep the `direction: input` rows, group them by `data_domains`, and read each row's state with the precedence in Tooling. Per requested kind:
   - At least one **connected** row → **connected**. Add "data may still be partial" when that row has `sync_in_progress: true`.
...
- A `need_reconnect` / `degraded` / `suspended` row was reported as `error` even when it had synced before.
