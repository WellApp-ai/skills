...
Call each list or read tool once per step, and render one widget card per turn, which is always the card carrying the turn's question. The cards refresh themselves. A card click executes server-side and prefills a message in the user's composer, so the card that carries the turn's question ends the turn, and the sent message is how the skill resumes. **This skill has one card, the period picker, and it always asks for the month.** It is never rendered without that question and nothing continues past it in the same turn, so this skill reads no period row for a month a hint or typed text already resolved: that read would draw the picker over a decided month. This sentence is the governing rule, so every step below follows it rather than restating it.

**In `mode: collect`** (a close or other start-a-run caller) the shape below is the same but the commit is not. Collect **exactly one** calendar month — refuse a multi-month pick in one line, since a run closes a single month — and treat the picker purely as month collection. Do not rely on the card click's server-side write as the selection: the deliverable is the month's `calendar_year` + `calendar_month`, handed back for the caller to pass to its start tool (`well_start_close`). Skip the "later reads omit their periods argument" narration entirely — it does not apply — and at the hand-off step hand control back to the caller rather than pointing at a period-scoped read. **A period card a `collect` run renders is never the confirmation its start tool rides on.** Step 3 reads no period row for a month a hint or typed text resolved, in either mode, which removes one cause of a card nothing is asked on. `show_close_readiness` is the other: it reads `well_list_periods` on this same path, and any call to it draws the picker in a host that renders one. That card must therefore carry a question of its own, and the caller's go-ahead is it: ask for the user's explicit yes to the collected month in that turn, and end the turn there, before `well_start_close`. A rendered card is not a confirmation; a click on it, or a typed yes, is. Everything else (workspace, hint resolution itself, the picker UX, the fiscal-coordinate arithmetic, the refusal of a month that has not ended) is unchanged.
...
3. **Read the hint before you read anything else.** A hint that resolves is written server-side at once: call `well_switch_workspace({ periods: [...] })` with the resolved months, say which months you took, read their period rows as the paragraph after this list requires, and go to step 6.
   ...
   **A month resolved without the picker reads no period row, in either mode.** The read would draw the period picker over months this skill has already written, so it would put a card on screen with nothing to ask, and the governing rule forbids that. Say which months you took and move on. The month therefore carries no `bank_transaction_count`, and that absence travels to the caller as an absence, never as a 0: a caller's bank step reads it as no evidence and asks the user, which is the safe answer for a month whose rows nobody has read. The same absence follows from `well_list_periods` being out of the toolset, and from a `bank_state` already `missing` or `error`, where the feed cannot have landed anything.
...
5. **Resolve the next message after the card.** In this order, and never by re-asking:
   ...
   - The message names a month or months in its own words → step 3's typed path: resolve, write with `well_switch_workspace({ periods })`, `resolution: user_picked`. **In `mode: select`, take each month's `bank_transaction_count` from the rows already read**, not from a fresh call: the route into this step called `well_list_periods` once for the picker, so the counts are in `period_rows`. A month typed outside the window that picker offered has no row, and it keeps no count: calling the tool again for it would draw the picker over a month the typed answer already decided. An absent count reaches the caller as an absence, never as a 0. In `mode: collect`, a message naming more than one month is refused the same way as a multi-month hint — take a single month, or ask which one.
...
- The hand-off, kept for the calling flow and never printed: `periods` — one entry per selected month, each with `calendar_year`, `calendar_month`, `fiscal_year`, `fiscal_period`, its label, `date_range` (`from` the first day, `to` the real last day), `is_complete`, and `bank_transaction_count` when the month's `well_list_periods` row carried one, which is the picker path alone (absent on a hint or a typed month, where no row is read; absent where the row did not carry the field, where `well_list_periods` is not in the toolset, and where a `missing` or `error` `bank_state` stopped the read: absent is not 0); `period_rows` — one entry per row `well_list_periods` returned, each with its `calendar_year`, `calendar_month` and `bank_transaction_count`, kept whenever that tool ran, which is the picker path alone. `periods` answers for the months SELECTED, `period_rows` for the months the picker READ, so a caller must never read one for the other. **Both lists belong to the workspace whose rows were read**, since a period row is per-workspace and neither list carries a `workspace_id`: a count read for one workspace is ABSENT for another, never 0; `period_label` for the whole selection — "to" only for consecutive months (e.g. "March 2026", "March to April 2026"), and "and" when the selection skips a month ("March and May 2026"), so a reader is never told a gap is covered; `has_activity` (`true`, `false`, or `unknown`); and `resolution` — `single`, `hint_matched`, `user_picked`, or `unresolved`. On `unresolved`, nothing else is kept. The selection itself lives server-side (`session.selected_periods`), which is why the later reads omit their periods argument; these keys are narration and routing vocabulary, and the hand-off travels as plain conversation, not as a data block.
...
- A month resolved by a hint or typed in text read no period row, in either mode, so no card was drawn over a month this skill had already written and its `bank_transaction_count` stayed absent, never reported as 0. No `collect` run drew a period card the caller could mistake for the user's go-ahead to start its run. A selection this conversation had already written was reused with no second read; where that write belonged to a different workspace, both count lists were handed back empty rather than repeated. A month typed after the picker took its count from the rows the picker already read, and `well_list_periods` was never called a second time.
- `period_rows` was kept for every run where the picker rendered, holding one entry per row it returned rather than one per selected month. Neither count list was presented as covering a workspace whose rows this pass did not read.
- Every card this skill drew was the period picker carrying the month question, and it ended the turn with one card-pointing line. No card was drawn over a month a hint or a typed answer had already resolved, and nothing continued past a card in the same turn.
...
"March" resolves to the most recent March that has ended: March 2026. Write it server-side, `well_switch_workspace({ periods: [{ calendar_year: 2026, calendar_month: 3 }] })`, and derive `fiscal_period = ((3 - 1 + 12) % 12) + 1 = 3` and `fiscal_year = 2026`. The hint resolved the month, so read no period row for it: that read would draw the picker over a month already written, with nothing to ask. March therefore carries no `bank_transaction_count`, and `probe: false` bars the `transactions` read as well, so `has_activity` is `unknown`. Answer: "Working on **March 2026**, fiscal year 2026, period 3. The month is complete, and the bank side is unconfirmed." Keep `resolution: hint_matched`, hand the absent count back as an absence, and point at `show-missing-invoices`, which will read this selection from the server, with no periods argument. The caller's bank step reads that absence as no evidence and asks the user for the bank, which is the answer for a month whose rows nobody read. Had the user picked March on the picker instead, that call's own row would carry `bank_transaction_count: 312`, `has_activity` would be `true`, and the caller's bank step would skip.
