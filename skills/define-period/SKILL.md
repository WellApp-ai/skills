...
is one. State the periods in text regardless — you cannot know whether anything drew
...
The click already wrote both months server-side. Derive both fiscal coordinates, then probe once: March and April are consecutive, so the two months form one interval, 2026-03-01 to 2026-04-30. Answer: "Working on **March and April 2026** — fiscal periods 3 and 4. March is complete; April is still running, so anything read for it is partial." Keep `resolution: user_picked` with both entries in `periods`. The later reads cover the whole selection without naming it.

### Example request

The user clicks **Validate** with March and May 2026 selected — April is left out — and sends the prefilled "Work on March 2026 and May 2026".
