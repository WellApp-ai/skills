...
5. **Resolve the next message after the card.** In this order, and never by re-asking:
   ...
   - The message names a month or months in its own words → step 3's typed path in full: resolve, write with `well_switch_workspace({ periods })`, read the period rows for those months as step 3 requires, `resolution: user_picked`. In `mode: collect`, a message naming more than one month is refused the same way as a multi-month hint — take a single month, or ask which one.
