...
3. **Read the hint before you read anything else.** A hint that resolves is written server-side at once: call `well_switch_workspace({ periods: [...] })` with the resolved months, say which months you took, read their period rows as the paragraph after this list requires, and go to step 6.
   ...
   **A month resolved without the picker still reads its period row.** When `well_list_periods` is in your toolset, call it once for the months you just wrote, scoped to those months where the tool takes a month scope, and keep each one's `bank_transaction_count` in the hand-off. That count is what a caller's bank step tests, and a hint or a typed month used to reach the caller without it: the caller then read the absence as no evidence and asked for a bank connection on a workspace whose month is full of bank transactions. The read renders the period picker card in a host that draws one, and there is nothing to pick on it, because the months are already written: say which months you took, point at that card in no line, and do not end the turn on it. A user who goes on to pick different months there is changing the month, which step 5 resolves like any other typed change. Two cases read no row and keep no count. `well_list_periods` absent from the toolset: no row exists, and absent is not 0. `bank_state` already `missing` or `error`: the feed cannot have landed anything, so no count can change the answer and none is read.
