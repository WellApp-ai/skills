...
Call each list or read tool once per step, and render at most one widget card carrying a question per turn. The cards refresh themselves. A card click executes server-side and prefills a message in the user's composer — rendering a card that carries the turn's question therefore ends the turn, and the sent message is how the skill resumes. **Two paths in this skill render a card that carries no question, and neither ends the turn**: a month already written by a hint or typed text, whose rows are read at step 3, and a picker whose turn the caller takes for a question of its own, at step 4. This sentence is the governing rule, so those two paths are stated here rather than left to be inferred from the step that owns them.

**In `mode: collect`** (a close or other start-a-run caller) the shape below is the same but the commit is not. Collect **exactly one** calendar month — refuse a multi-month pick in one line, since a run closes a single month — and treat the picker purely as month collection. Do not rely on the card click's server-side write as the selection: the deliverable is the month's `calendar_year` + `calendar_month`, handed back for the caller to pass to its start tool (`well_start_close`). Skip the "later reads omit their periods argument" narration entirely — it does not apply — and at step 8 hand control back to the caller rather than pointing at a period-scoped read. **Read no period row for a month a hint or typed text resolved.** Step 3 requires that read in `mode: select`, where a caller's bank step tests the count; in `collect` no caller reads it. **And a period card a `collect` run renders is never the confirmation its start tool rides on.** Dropping that read removes one cause of an incidental card, not every cause: `show_close_readiness` reads `well_list_periods` on this same path, and any call to it draws the picker in a host that renders one. So a caller must take the user's explicit yes before `well_start_close` whenever the month came from a hint or typed text, whatever appeared on screen. A rendered card is not a confirmation; a click on it is. Everything else (workspace, hint resolution itself, the picker UX, the fiscal-coordinate arithmetic, the refusal of a month that has not ended) is unchanged.
...
2. **Confirm the workspace, and reuse a selection that already exists.** Require `workspace_id`. If the caller did not pass one, run `define-workspace` and take its hand-off; never resolve a workspace here — and never ask for it in text. Read `fiscal_year_start_month` from the same hand-off; when it is null, use `1` and say the workspace has no fiscal-year setting yet so you assumed a calendar year. When the session holds `selected_periods` that THIS conversation itself wrote (its own card click or typed months, earlier in the conversation) and the user is not asking to change the month, use that selection silently and go to step 6 — never re-ask a choice this conversation already made. **This path reads no period row, and needs none in the workspace the rows were read in.** Every path that writes a selection reads its rows (step 3 for a hint or a typed month, step 4's picker call for a click), so a selection THIS conversation wrote already put its `bank_transaction_count` in the caller's hand; reading again draws a second picker for a count the caller holds. Where that write kept no count, `well_list_periods` absent or a `missing` / `error` `bank_state`, it stays absent here too, and absent is not 0.
   - **A selection written for a DIFFERENT workspace carries no count for this one.** Period rows are per-workspace, and neither `periods` nor `period_rows` names the workspace it was read in. So on a caller's multi-workspace loop, where a later pass reuses a selection an earlier pass wrote, hand both lists back EMPTY for this pass rather than repeating the earlier workspace's counts. That fails safe: the caller's bank step reads an absent count as no evidence and asks the user, which is the right answer for a workspace whose rows nobody has read. Repeating them would skip the bank step on another entity's evidence. A `selected_periods` present at conversation start that this conversation did not write is another conversation's leftover: ignore it, never mention it — "already recorded" / "already selected" is forbidden phrasing — and continue to steps 3–4 as if it were unset; the user's fresh pick overwrites it server-side.
...
3. **Read the hint before you read anything else.** A hint that resolves is written server-side at once: call `well_switch_workspace({ periods: [...] })` with the resolved months, say which months you took, read their period rows as the paragraph after this list requires, and go to step 6.
   ...
   **A month resolved without the picker still reads its period row, in `mode: select` only.** When `well_list_periods` is in your toolset, call it once for the months you just wrote, scoped to those months where the tool takes a month scope, and keep each one's `bank_transaction_count`. That count is what a caller's bank step tests, and a hint or a typed month used to reach the caller without it: the caller read the absence as no evidence and asked for a bank connection on a workspace whose month is full of bank transactions. The read draws the period picker in a host that renders one, and there is nothing to pick on it, since the months are already written: say which months you took, point at that card in no line, and end no turn on it. A user who picks different months there is changing the month, which step 5 resolves like any other typed change. Three cases read no row and keep no count: `well_list_periods` absent, where no row exists and absent is not 0; a `bank_state` already `missing` or `error`, where the feed cannot have landed anything so no count can change the answer; and `mode: collect`, where the count has no consumer and the card this read would draw sits in the turn that asks for a considered yes to a write.

4. **With no usable hint, end the turn on the picker.**
   - When `well_list_periods` is in your toolset, call it (with `workspace_id`, and `title` / `subtitle` when supported). Its result renders the period picker card — do not restate the periods under it and do not ask "which month?" in text; the card is the question. End the turn with one short line: pick the month or months on the card, then send the message it prepares. Use `purpose` when the caller gave one. Nothing else in the turn, unless the sub-bullet below applies. The **Use** click writes the selection server-side and prefills "Work on <Month Year> and <Month Year>" in the composer. **In `mode: collect`**, ask for a single month on the card; if the pick comes back with several, refuse in one line — a close runs a single calendar month — and ask which one to keep.
     - **Unless the caller takes this turn for a question of its own.** A caller that reads the rows this call returned and finds something it must settle BEFORE the month is worth picking keeps the turn: Well's fetch-missing-invoices flow asks for a bank connection when no returned row holds a bank transaction, because a month picked against no bank data buys the user nothing. Then render the picker, point at it in no line, end no turn here, and leave the month question for the turn after the caller's answer. The card stays on screen, so that later turn points at it rather than calling `well_list_periods` twice. Where the caller settles nothing first, this step ends the turn on the picker exactly as above.
...
5. **Resolve the next message after the card.** In this order, and never by re-asking:
   ...
   - The message names a month or months in its own words → step 3's typed path: resolve, write with `well_switch_workspace({ periods })`, `resolution: user_picked`. **In `mode: select`, take each month's `bank_transaction_count` from the rows already read**, not from a fresh call: every route into this step has called `well_list_periods` once, so the counts are in `period_rows` and a second call would draw a second period picker. Call it again, scoped to the months in question, only for a month no row already covers, which is a month typed outside the window the picker offered. That second call is this skill's one authorised repeat of the tool, and it lands in a later turn than the picker call, so no turn holds two of its cards. In `mode: collect`, a message naming more than one month is refused the same way as a multi-month hint — take a single month, or ask which one.
...
- The hand-off, kept for the calling flow and never printed: `periods` — one entry per selected month, each with `calendar_year`, `calendar_month`, `fiscal_year`, `fiscal_period`, its label, `date_range` (`from` the first day, `to` the real last day), `is_complete`, and `bank_transaction_count` when the month's `well_list_periods` row carried one, on the hint and typed paths as much as on the picker path (absent where the row did not carry it, where `well_list_periods` is not in the toolset, and where a `missing` or `error` `bank_state` stopped the read: absent is not 0); `period_rows` — one entry per row `well_list_periods` returned, each with its `calendar_year`, `calendar_month` and `bank_transaction_count`, kept whenever that tool ran. `periods` answers for the months SELECTED, `period_rows` for the months READ. They hold the same months only on the hint path, where the read is scoped to the months already written. Wherever the picker ran, including a month typed after it, `period_rows` is every month the picker offered and `periods` is the subset selected or typed, so a caller must never read one for the other. `period_rows` is also the only hand-off fact a caller can read BEFORE a month is selected. **Both lists belong to the workspace whose rows were read**, since a period row is per-workspace and neither list carries a `workspace_id`: a count read for one workspace is ABSENT for another, never 0; `period_label` for the whole selection — "to" only for consecutive months (e.g. "March 2026", "March to April 2026"), and "and" when the selection skips a month ("March and May 2026"), so a reader is never told a gap is covered; `has_activity` (`true`, `false`, or `unknown`); and `resolution` — `single`, `hint_matched`, `user_picked`, or `unresolved`. On `unresolved`, nothing else is kept. The selection itself lives server-side (`session.selected_periods`), which is why the later reads omit their periods argument; these keys are narration and routing vocabulary, and the hand-off travels as plain conversation, not as a data block.
...
- With no hint, the picker rendered and the turn ended with one card-pointing line, unless the caller took that turn for a question of its own; no wait tool was called in that turn — or in any turn before the picker existed — and in a host that renders the card, no text question replaced it. In a text-only host, the three-month list and its single question stand in for the picker; when `well_list_periods` was absent entirely, the last-complete-month proposal did — both as step 4 allows.
...
- In `mode: select`, a month resolved by a hint or typed in text still read its period row wherever `well_list_periods` was in the toolset, so its `bank_transaction_count` reached the hand-off. That read pointed at no card and ended no turn. Where the tool was absent, where `bank_state` was already `missing` or `error`, or where the mode was `collect`, no row was read and the count stayed absent, never reported as 0. No `collect` run drew a period card the caller could mistake for the user's go-ahead to start its run. A selection this conversation had already written was reused with no second read, because the write that made it read the rows itself; where that write belonged to a different workspace, both count lists were handed back empty rather than repeated. A month typed after the picker took its count from the rows already read, and called `well_list_periods` again only for a month no row covered.
- `period_rows` was kept for every run where `well_list_periods` ran, holding one entry per row it returned rather than one per selected month, so a caller could read coverage across the picker before any month was picked. Neither count list was presented as covering a workspace whose rows this pass did not read.
- Where the caller took the turn on the picker path for a question of its own, this step rendered the picker, pointed at it in no line, ended no turn, and left the month question for the turn after the caller's answer. On every other picker path it ended the turn on the card with one card-pointing line.
...
"March" resolves to the most recent March that has ended: March 2026. Write it server-side, `well_switch_workspace({ periods: [{ calendar_year: 2026, calendar_month: 3 }] })`, and derive `fiscal_period = ((3 - 1 + 12) % 12) + 1 = 3` and `fiscal_year = 2026`. The hint resolved the month, and `well_list_periods` is in the toolset, so read March's own period row anyway, scoped to that month, and keep its `bank_transaction_count: 312` in `periods` and in `period_rows`: that count is the only bank signal the caller's bank step accepts, and the card the read draws asks nothing, since March is already written. Run no probe: the row's count answers the activity question, and `probe: false` bars the `transactions` read in any case. A count above 0 on a selected month sets `has_activity: true`. Answer: "Working on **March 2026**, fiscal year 2026, period 3. The month is complete and holds bank activity." Keep `resolution: hint_matched`, hand back the count, and point at `show-missing-invoices`, which will read this selection from the server, with no periods argument. Had `well_list_periods` been absent, no row would exist: both counts would stay absent, `has_activity` would be `unknown`, and the answer would say the bank side is unconfirmed.
