...
- No `well_switch_workspace` call pinned a workspace here — every call this skill made carried `periods`, and none named a workspace to pin. The caller's pin stood for the whole step, and the step after this one opened on it without a re-pin.
...
- A month that has not ended was refused; several months named resolved to one written selection, oldest first; a quarter name was read as a calendar quarter and its three months were named when the fiscal year does not start in January.
