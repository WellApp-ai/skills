...
- `has_activity` came from `bank_transaction_count` wherever the period rows carried it, and no probe ran in that case, under `probe: false`, or behind a `missing` or `error` `bank_state`. From a probe it is `false` only on an empty read behind a `connected` feed. It is `unknown` — not `false` — when a selected month carried no count, when the read failed, when `bank_state` said the feed is missing, in error, or still `connecting`, and when no `bank_state` was passed at all. The probe ranged `executed_at` alone, over the selected months' own intervals — one interval per run of consecutive months, `_or`-ed together — and never across a gap the selection does not cover.
