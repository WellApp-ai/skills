...
**Duplicate invoices are not checked here.** The canonical control point is `EINV-02` at gate 8,
which alone anchors the grouping on `workspace_pk`. `GRAPH-duplicate-invoice-identity` applied the
same predicate to the same population at gate 4 in the **exhaustive** bucket while
`ING-duplicate-invoice-number` did it at gate 2 in **complete** — one duplicated bill counted three
times, in both buckets. Its `source_workspace_connector_pk` segmentation and its second
(`issuer`, `reference_number`, `grand_total`, `issue_date`) arm move to `EINV-02`.
