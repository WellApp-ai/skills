...
| `ING-balance-series-continuity-break` | Gap between adjacent periods | fetch per account `orderBy balance_at_from asc` (one response — there is no pagination, `iteration-protocol.md` B.1); compare each `balance_at_to` to next `balance_at_from`, and `closing_booked` to next `opening_booked` | any mismatch | red | complete | **`verification_error` validates *within* a period and cannot see a missing period *between* two clean ones** — exactly what cash-balance-trend reports as a real business trend |
...
| `ING-duplicate-transaction-external-id` | Same source txn ingested twice | fetch `transaction_external_id`, `workspace_connector`, `executed_at`, `instructed_amount`; group on (connector, external_id) client-side | any tuple > 1 with null `deleted_at` — a partial unique index exists, so a hit means it was bypassed or the id is null | red | complete | double-counted outflow inflates burn → runway under-reported, category double-charged |
...
| `ING-fx-rate-missing-for-live-currency` | Live currency has no rate | distinct currencies from `accounts.currency`, `invoices.local_currency`, `transactions.instructed_amount.currency`; per currency `exchange_rates` where `source_currency _eq <cur>`, `rate_date _gte now-7d` | `totalCount = 0` — **but the home/non-home partition is INCONCLUSIVE**: no home-currency field exists on `workspaces` (`mcp-surface-limits.md`), and it must never be inferred from the commonest transaction currency, so the home currency (which correctly has no `exchange_rates` row) cannot be excluded. Report the per-currency zeros and mark the partition unevaluable rather than firing an unearned red | red on a non-home currency, INCONCLUSIVE on the partition | complete | fx-exposure must abandon; others blend or split silently |
...
**`BANK-txn-no-external-id-dup` is removed; the canonical is `ING-duplicate-transaction-external-id`
above.** The same (`source_workspace_connector_pk`, `transaction_external_id`) grouping over the same
population was carried at gate 3 as well, counting one re-ingestion twice; ingestion owns
re-ingestion, so the check is counted once here. The reporting rule it carried moves with it:
`idx_transactions_connector_external_id_unique` is **partial**, so a collision inside its predicates
is structurally impossible — **report the count of rows the index does not reach** (null or blank
`transaction_external_id`, or null `source_workspace_connector_pk`), not the structurally-zero
collision count. **Cross-connector** id reuse is a different population, stays
`BANK-txn-no-cross-connector-dup`, and is classified `review`, never `duplicate` — do not auto-merge
it.

**Duplicate invoices are not checked here.** The canonical control point is `EINV-02`, which
alone anchors the grouping on `workspace_pk` and carries the rationale that the invoice-number index
is **not** unique. `ING-duplicate-invoice-number` (gate 2, complete) and
`GRAPH-duplicate-invoice-identity` (gate 4, exhaustive) applied the same predicate to the same
`invoices` population under two more ids and two different **buckets**, so one duplicated bill
inflated the red count three times and landed in COMPLETE *and* EXHAUSTIVE at once. Both are removed;
`EINV-02` carries `GRAPH-`'s `source_workspace_connector_pk` segmentation and its second
(`issuer`, `reference_number`, `grand_total`, `issue_date`) arm.
...
| `ING-balance-period-cadence-gaps` | Missing days/months in series | fetch per account; derive modal period length, scan for longer intervals. Tolerance 16: a non-publishing weekend/bank holiday is **not** a hole | interval > 1.5× modal, or fewer periods than the window implies | red | exhaustive | the stated window overstates real coverage |
