...
| `ING-duplicate-invoice-number` | Same invoice twice | fetch `invoice_number`, `grand_total`, `local_currency`, `issue_date`, `issuer.name`; group on (number, issuer) client-side | count > 1 for non-null number | red | complete | bills-due shows the bill twice; rank-clients-by-ltv inflates LTV |
