...
- `period` — optional, the **composed-mode** input. A caller that has already fixed the month (the `fetch-missing-invoices` flow in composed mode, driven by a close) passes it as `{ calendar_year, calendar_month }` or `{ fiscal_year, fiscal_period }`, both halves together. When present, forward it verbatim to `well_list_missing_invoices` instead of relying on a server-side selection — the composed caller wrote none. Absent, the skill behaves as above (no period argument, session selection).
