...
4. **Report it once, and let the card take the pick.**
   - In an MCP-Apps host the result already renders the missing-invoices card: the counterparty rows, each carrying one collection-method badge per entry of its `available_modes` (**Deploy agent**, **Connect tools**, **Upload**), grouped by the person the spend is attributed to when that data is there and flat when it is not, each row carrying a checkbox and its own transaction table. Do not restate the rows in text under it, and do not re-group them yourself. Give the summary line — how many counterparties in each mode, and the total — then the coverage line from step 6 and one line telling the user to tick the vendors to chase and click Continue. Nothing else: no per-row commentary and no restated rows.
...
is one. Do not compose a second rendering of rows the tool already returned.
...
- The categorized-only coverage line was stated, even on an empty list, with each non-zero `dropped_groups` counter and any `hints`, and with the still-moving caveat when the selection holds a month that is still running. The line quantified no examined-transaction figure, and `transaction_count` was never presented as the transactions Well read.
...
- The turn that carries no tick request ends with the next-step pointer — the empty list, the unavailable list, and the turn after the pick is written (`categorize-counterparties`, then `connect-tools` before `deploy-agents` when the pick names a connector Well carries, otherwise the caller or a question). The card turn ends on the tick request instead.
