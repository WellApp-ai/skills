...
Keep: the period as the result expressed it — the single-month fields when the selection held one month, `periods_covered` plus the per-month `months` totals when it held several, so the later steps are never told one month for a list that spans several — `counts` per mode, `total_base_amount`, `transaction_count`, `agent_candidates` (the `mode: agent` rows grouped by `matched_provider_name`, unmatched under `"unknown"`, each group with its counterparties and their `company_id`, summed `tx_count`, summed non-null amount, and the shared `matched_connector_service_id` as `provider_id`), `selection` (the picked `company_id` / `matched_connector_service_id` pairs), `coverage_note`, and `resolution` — `listed`, `empty`, or `unavailable`. Then continue to step 7's close in the same turn; step 6 runs only when the user asks for it.

### Step 6 — Categorization gate (only on request)

**This step has no automatic trigger.** The gap-list result carries no count of the categorized expense transactions Well examined, so nothing in it says the list is thin. `well_list_missing_invoices`' `transaction_count` counts the transactions missing their invoice, and `transaction_count: 0` therefore means nothing is missing — the opposite of thin. So never open this step on your own, and never quantify the coverage.

Run it when the user asks, and never silently: say in one line that the gap list covers the period's categorized expense transactions only and may hide gaps behind spend that is not categorized yet, ask whether to categorize the counterparties behind the rest first, and proceed only on the user's yes — the card it opens saves each pick immediately.
...
| 5 / 7 | resolution | `empty`, `dropped_groups.unknown` + `unnamed_company` = 0 | **stop and celebrate** — every categorized expense from a supplier has its invoice; recap without a preview |
| 5 / 7 | resolution | `empty`, `dropped_groups.unknown` + `unnamed_company` > 0 | **stop, celebrate nothing** — name the unattributed group count, claim nothing over it; recap without a preview, and never as a complete period |
| 5 / 7 | request | the user asks to categorize first | go to 6 (ask first) |
| 5 | resolution | `listed` | continue to 7 in the same turn |
...
| 7 | rows | unchanged after step 6's card | say the categorization did not move the list; the tick-and-Continue line still ends the turn |
...
- Step 6 ran only on the user's request, only after an explicit yes, wrote only rows the user confirmed, and the list was re-read exactly once after step 6's card rendered. No answer called the list thin, and `transaction_count` was never presented as the transactions Well examined. An all-categorized read was stated in half a sentence and the flow continued to step 7's close line in the same turn.
...
**Two entities.** "March across FR and US" → `well_switch_workspace({ workspace_ids: [fr, us] })` pins FR, queues US, and step 1 ends its turn naming the sequence. Run 2→8, recap. Read `session.workspace_queue`, re-pin, end that turn on the picker, then run 2→8 again — `empty` with every `dropped_groups` counter at 0 celebrates at step 5. Two recaps, nothing added together.

**Categorize first, on request.** Step 5 lists nine counterparties, and the user asks whether uncategorized spend hides more. Say the list covers the period's categorized expense transactions only, quantify nothing, and ask whether to categorize the counterparties behind the rest first; on yes, step 6's card, one re-read at step 7, and step 8 previews once the vendor pick arrives from the refreshed card.

**An empty list that is not a complete period.** Step 5 comes back `row_count: 0` with `dropped_groups: { bank_internal: 1, unknown: 0, unnamed_company: 2 }`. Two groups of categorized spend carry no named counterparty, so step 7 celebrates nothing: report the two groups, say the flow can neither list nor chase them, say the period also holds bank operations with no counterparty and quote no number for them, and recap without a preview. Never say the period has every invoice.
