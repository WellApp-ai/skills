...
- `unknown` plus `unnamed_company` is 0 → **stop and celebrate**: every categorized expense from a supplier has its invoice. Name a non-zero `bank_internal` count as bank operations with no supplier to invoice them. Nothing to tick, no pointer.
...
**An empty list that is not a complete period.** Step 5 comes back `row_count: 0` with `dropped_groups: { bank_internal: 1, unknown: 0, unnamed_company: 2 }`. Two groups of categorized spend carry no named counterparty, so step 7 celebrates nothing: report the two groups, say the flow can neither list nor chase them, name the bank operation as having no supplier to invoice it, and recap without a preview. Never say the period has every invoice.
