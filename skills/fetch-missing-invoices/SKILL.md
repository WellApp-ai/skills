...
- `well_wait_for_selection({ kind, timeout_s? })` — the click read, legal only AFTER this conversation has rendered the matching card: its one job is reading the click the user made on that card when their next message is not its prefill. Never at step start, never before the card exists, never as a probe for a pin, selection, or ack — read that from `well_list_workspaces`' `session` block or this conversation's history; a fresh conversation has none, so an unresolved step renders its card at once, no call before the render. `kind` is `"workspace"`, `"periods"`, `"connect_ack"`, or `"bank_ack"`. An already-made selection returns instantly as `{ status: "selected", selection, already_set: true }` — workspace: `{ workspace_id, workspace_queue }`; periods: `{ periods }`; acks: `{ acknowledged: true }`; when nothing is set yet it waits briefly (default 10 seconds) and returns `{ status: "no_selection_yet" }`, a normal result, not an error.
...
- Step 6 ran only on a thin list or on request, only after an explicit yes, wrote only user-confirmed batches, and the list was re-read exactly once after `updated`. An all-categorized read was stated in half a sentence and the flow continued to step 7's go line in the same turn.
...
**Happy path, one turn per card.** "Fetch the invoices I'm missing for March." One workspace; Qonto and Pennylane connected. → Step 1 resolves silently. Step 2 renders the connect card, one coverage line, turn ends; the prefilled "Continue" arrives — acknowledge in half a sentence, move on. Step 3 renders the bank card ("Bank: connected — Qonto"), turn ends; the "Continue" prefill arrives — move on. Step 4 writes March 2026 from the hint via `well_switch_workspace({ periods })`. Step 5 calls `well_list_missing_invoices({ workspace_id })` — 12 counterparties, `transaction_count: 41`, not thin — and step 7 ends the turn: "12 suppliers, €18,430 still without an invoice — say **go** and I'll preview the agents." The user says "go"; step 8 previews on that message, then the recap and the no-launch sentence.

**A message that is not the prefill.** At the bank card the next message is "keep going" instead of the prefill. The card is on screen, so call `well_wait_for_selection({ kind: "bank_ack", timeout_s: 10 })` once — the click landed before they typed, so the ack returns with `already_set: true` — and resume at step 4 without re-asking anything. Had it returned `no_selection_yet`, one line asking to click Continue ends the turn.

**Two entities, one click.** "What am I missing for March across FR and US?" → the compound hint matches both: `well_switch_workspace({ workspace_ids: [fr, us] })` pins Acme SAS and queues Acme Inc. Run 2→8 on Acme SAS (acks and go as above), recap it. Then read `session.workspace_queue`, `well_switch_workspace({ workspace_id: us })`, run 2→8 again — March comes back `empty` with `transaction_count: 23`, so that pass celebrates at step 5; its recap has no preview. Two recaps, nothing added together.

**Thin list.** March returns `empty` with `transaction_count: 0` → say the list rests on 0 categorized transactions and may hide gaps, ask whether to categorize first. On yes, run step 6's confirmed batches, re-read once at step 7 (its go line ends the turn), preview at step 8 on the go.
**Tool unavailable.** `well_list_missing_invoices` absent → steps 1-4 run; step 5 is `unavailable`: say the server does not expose the tool yet and the list is never approximated from raw transactions; stop — no categorization, no preview — and recap workspace, period, and coverage.
