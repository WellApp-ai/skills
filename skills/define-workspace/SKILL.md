...
Call each list or read tool once per step, and render one widget card per turn, which is always the card carrying the turn's question. The cards refresh themselves. A card click executes server-side and prefills a message in the user's composer, so the card that carries the turn's question ends the turn, and the sent message is how the skill resumes. **Nothing continues past a card in the same turn, and `well_list_workspaces` draws its picker on every result.** One account asks nothing about the workspace at all, the one holding exactly one workspace, and its picker shows a single tile: step 3 names the entity in one line, points at no card, and still ends the turn. This sentence is the governing rule, so that path is stated here rather than left to be inferred from the step that owns it.
...
3. **Resolve without asking when you can.**
   - Exactly one workspace → use it. `resolution: single`. Say which one in one line; do not ask for confirmation and do not call `well_switch_workspace` — with one authorized workspace there is nothing to choose between. The picker renders here too, over that one tile, and asks nothing: point at no card and wait on none. End the turn there all the same, because the card is on screen and you cannot tell a host that drew it from one that did not.
...
- With exactly one workspace, nothing was asked: no line pointed at a card, no wait tool ran, and the answer named the entity in one line before the turn ended.
...
Call `well_list_workspaces()`, get one workspace, and name it without asking: "Working in **Acme SAS** (FR, EUR)." Its picker shows one tile and asks nothing, so no line points at it and no wait tool runs. The turn ends there. Keep `resolution: single` and the workspace's `has_bank_transactions` for the flow — nothing more is printed.
