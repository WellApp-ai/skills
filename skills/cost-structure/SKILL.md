...
4. **Get the breakdown.** Call `well_get_cost_structure()`. Pass `year` and `month` only if the user named a past month. It returns `entries` (an array of `{ category, amount, pct }`, sorted by amount descending), `currency`, the `period_start`/`period_end` bounds, and a `rung`.
   - **This call is the only analytics tool this skill may call.** `well_get_cost_structure`'s own response carries every figure this answer states — the `entries` with their shares, the currency, the period bounds and the `rung`. Do not call `well_get_runway`, `well_get_burn`, `well_get_cash_position`, `well_get_cash_forecast` or `well_get_cash_flow_bridge` — not for a comparison, not for a series, not for one number in a sentence. Each of them draws its own card, so a second call renders a second block beside the one the user asked for, answering a question they did not ask. A month-over-month comparison is the most tempting of these and the clearest violation: it needs a second read of another block's tool, and the trend it would narrate is another skill's answer. If the sentence you want needs a figure this payload does not carry — a monthly burn rate, a change against an earlier month, unpaid obligations rather than settled spend — that figure is another skill's answer: name that skill, as the Output requirements already say, and stop.
...
- Exactly one analytics tool call was made — `well_get_cost_structure` — and no other block's analytics tool (`well_get_runway`, `well_get_burn`, `well_get_cash_position`, `well_get_cash_forecast`, `well_get_cash_flow_bridge`) was called at all, for any reason, including to source a figure for a sentence.
