...
The read also hands back `targets` (each `{ kind, id, name, workspace_count? }` — this workspace, or a workspace group the user belongs to), `roles` (`admin` or `member`), `me_person_id`, and `resolved_workspace`. Every candidate carries `person_id`, `name`, `email`, `avatar_url`, and its `state`.
