...
5. **Query outstanding receivables.** Call `well_get_schema({ root: "invoices" })` (always, even if queried earlier in the session for a different purpose — this skill relies on `payment_status`, a separate dimension from lifecycle `status`, and field behavior can vary by connector). Query `invoices` where `issuer_company_id` matches the confirmed own-company identity set and `payment_status` is `unpaid` or `partial`. Include `receiver.name`, `grand_total`, `balance_due`, `local_currency`, `due_date`, `issue_date`, `invoice_number`.
   ...
   - **Don't let an equality filter hide rows.** A filter on `issuer_company_id` silently drops invoices where it is `null`. Query that bucket separately and age those invoices as labeled rows ("unattributed — issuer not recorded") inside the buckets, not in a footnote: a large unattributed receivable sitting in 90+ is exactly what this skill exists to surface. Handle invoices whose issuer and receiver are the same company the same way — surface them labeled, don't drop them.
...
- Invoices with a null `issuer_company_id`, or with issuer equal to receiver, were aged as labeled rows inside the buckets — not dropped by the equality filter, and not demoted to a footnote.
