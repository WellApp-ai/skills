...
description: Preview what Well would fetch for the vendors the user picked — which agents would run, over which counterparties and transactions, which rows need a manual upload, and which providers need connecting — then hand those vendors to the Well app. Nothing collects anything yet. The preview card carries a checkbox per vendor and a select-all, and its primary action opens the acquisition link for the providers the user ticked, whose acquisition page lists them and says Well cannot collect them yet. Use when the user asks to fetch, collect, or chase the invoices they are missing, says "launch the agents", "go get those invoices", "deploy the collectors", or when the fetch-missing-invoices flow reaches its last step after the missing rows have been listed and picked. Do not use to run a collection from the chat, to invoke a connector's own actions, to create or edit an invoice, to connect a provider, or to list which invoices are missing in the first place.
---
...
**Nothing collects these invoices.** This skill and its card start no agent, open no browser session
and queue no task. The card's primary action opens the acquisition link, which hands the providers
the user ticked to the Well app. The acquisition page there lists those vendors and states that Well
cannot collect them yet. So the honest sentence is not "the Well app starts the run" — it is
"nothing has started, and Well cannot collect these invoices yet". This is the last brick of Well's
fetch-missing-invoices flow.
...
- The user wants a fetch result, a downloaded file, or a status update on a running collection — no
  Well surface starts one yet, so there is nothing to follow or report on. The acquisition link its
  card opens leads to a page that lists the picked vendors and collects nothing; say that instead of
  promising progress.
...
- The vendor selection — the pick the missing-invoices card wrote, carried in that hand-off's
  ...
  workspace than the pinned one is not this pass's pick. **A pick is trustworthy only for the
  gap-list card now on screen.** A pick taken on an earlier card — before a fresh gap-list read, or
  before a change of months — names rows the user is no longer looking at, so it is not this pass's
  pick either. Send the flow back to `show-missing-invoices` for a fresh tick rather than previewing
  on it.
...
- `well_preview_invoice_fetch` — **when it is present in your toolset.** Input: `workspace_id`
  ...
  identifier a run routes on, `domain` is the host the acquisition link carries,
...
- `scoped_to_selected_counterparties` — **the field that says what the result covers.** It is
  present and `true` when the server held a pick for this workspace, and then every route in the
  result covers the picked vendors only. It is absent when the server held no pick, and then the
  result covers every gap of the months read. Read it before you describe the scope; never infer the
  scope from the row count.
- **The preview card, in MCP-Apps hosts.** The result renders the previewed vendors with a checkbox
  per row and a select-all, and its primary action opens the acquisition link for the hosts of the
  providers still ticked. The card starts nothing, and the acquisition page the link opens collects
  nothing. So end your turn on the card and let the user tick and open it; do not restate the rows it
  draws, and never describe the link as a collection that started.
- **When the tool is absent**, no tool call is needed at all. Derive the same preview from the
  ...
  no `acquisition_url`, so it carries no acquisition link to open: give the workspace link instead
  and say Well cannot collect these invoices yet.
...
3. **Build the preview, and keep it to the pick.**
   - Tool present → call `well_preview_invoice_fetch({ workspace_id })` — no periods argument; the
     ...
     `counts`, `scoped_to_selected_counterparties` and `acquisition_url` as they come, `provider_id`
   ...
   - **Flag absent → the result covers the whole period, not the pick.** The server held no pick for
     ...
     counterparty, which the tool's rows do not, so they are the only identifier-matched way to reach
     the picked set. With no hand-off to narrow from, report the plan as covering the whole period
     and say so; never present a period-wide result as the pick.
...
4. **Say what would run — one line per agent, in the user's language.** Read the user's language from
   ...
   mechanics — **Deploy** opens the acquisition link for the vendors still ticked, **Continue**
   stands in its place when there is no link to open, and **Keep for later** leaves the card be — so
   ask the user to confirm the vendors and deploy, and leave the labels to the card. Where no card is
   drawn, give the tool's `acquisition_url` when the tool ran and the workspace link otherwise, and
   say the acquisition page lists the picked vendors and collects nothing yet. Append no query
   parameter of your own to either link.
...
7. **State plainly that nothing collects these invoices.** One sentence of its own, not a
   parenthesis: no agent has started, no task is queued, no browser session is open, and Well cannot
   collect these invoices yet — the acquisition page lists the vendors the link carried and does
   nothing else. Say it even where the card names agents. Never claim a launch, a result, a
   downloaded invoice, a success rate, or an ETA — you have none of those, and neither this skill,
   its card, nor the page the link opens can produce them. When the user reports that the link
   opened, say the page lists the vendors they picked and collects nothing; do not narrate a run
   nothing started, and do not report a refused link as a launch.
...
- One line for the rows to upload by hand, and one line for the providers still to connect. Both
  ...
  `domain`, for the vendors the acquisition link cannot name. On `nothing_to_do` the single
...
- One line stating what the plan covers — the picked vendors only when
  `scoped_to_selected_counterparties` is true, the whole period when the result carried no pick and
  no hand-off narrowed it.
- One line asking the user to confirm the vendors and deploy from the card — or, where no card is
  drawn, carrying the acquisition link itself.
- One plain sentence stating that no agent has started, no task is queued, no browser session is
...
- The hand-off, kept for the calling flow and never printed: `workspace_id` — the one this skill
  ...
  acquisition link when the tool returned one, else null; `scoped_to_selected_counterparties` — true
  when the tool scoped the result to the pick, absent otherwise; the `coverage_note` —
  ...
  on and `domain` is the host the acquisition link names; `provider_id` is null only when neither the
...
- End with a one-line pointer to the next step. Hand the block back to the caller — the
  ...
  to run the collection, point at the card's Deploy action — or at the acquisition link where no card
  is drawn — and say plainly that Well cannot collect these invoices yet. Never offer to run a
  collection from the chat, and never promise one anywhere else.
- Beyond the per-agent, upload, unmatched, connect, coverage, scope, deploy and nothing-collected
...
- Any claim that an agent ran, is running, or produced a result — including a percentage, a file, or
  an ETA — any claim that the acquisition page collects the invoices, and any claim that a link the
  host refused started a run.
...
- A vendor promised to the run when its `domain` is null, or an acquisition link you built yourself.
...
- The pick was in hand before anything was previewed: a `selection` from the caller or the session,
  matched on `company_id`, with a pending pick — or a pick belonging to an earlier gap-list card —
...
- The turn ended on the card with one line asking the user to confirm the vendors and deploy — or,
  where no card was drawn, carrying the acquisition link from the tool's `acquisition_url` or the
...
- The answer contains one plain sentence stating that nothing has started and that Well cannot
  collect these invoices yet, with the acquisition page named as a page that lists the picked
...
- The hand-off facts were kept — `workspace_id`, the period, `run_mode: preview`,
  ...
  `acquisition_url`, `scoped_to_selected_counterparties`, `coverage_note`, and `resolution` — and no
...
- The answer ends with the hand-back to the caller, and any request to run the collection was
  answered with the card's Deploy action or the acquisition link plus the fact that Well cannot
  collect these invoices yet, never with a run from the chat or a run promised elsewhere.
...
> Well ne peut pas encore récupérer ces factures. Ouvrez l'espace de travail dans l'app Well —
...
only. The preview card renders those vendors with their checkboxes, its select-all, its Preview badge
and its Deploy action. Do not restate the rows. Say one line per agent — provider, count, nothing
started yet — then the upload line, the connect line, the line saying the plan covers the picked
vendors only, the coverage line carrying the tool's `hints`, one line asking the user to confirm the
vendors and deploy, and the plain sentence that Well cannot collect these invoices yet. Carry each
agent's `provider_id` and `domain`, plus the envelope's `acquisition_url` and
...
Say plainly that Well cannot collect these invoices yet — from the chat or anywhere else. Deploy on
the card opens the acquisition link for the vendors still ticked, and the acquisition page lists
them and collects nothing. Where no card is drawn, give the acquisition link from `acquisition_url`,
or `<well-app-base-url>/workspaces/<workspace_id>` when the preview carried none. Do not call any
tool, do not promise a run from the chat or from the Well app, and do not re-emit the preview as
...
plain sentence that Well cannot collect these invoices yet. Never write "Agent ready for unknown".
