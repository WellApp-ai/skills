...
- **Keep skill size in view while composing — and hold atoms to a stricter budget than skills.** `make validate` runs `scripts/check-content-size.mjs`, which warns (never fails the build) on any `skills/*/SKILL.md` over a 30,000-token ballpark estimate (plain character count / 4, not a real tokenizer — 30k is ~3% of Claude's 1M-token context), and any `atoms/*/CONTENT.md` over **10% of that** — 3,000 tokens. The tighter atom budget isn't arbitrary: an atom is inlined verbatim into every skill that composes it, so its size is paid by each of them, not once. This is a hygiene ceiling, not a quality-degradation claim — no source ties a specific token count to measured output quality at the sizes these files actually reach (checked directly: the token-length-degradation research that exists — Chroma's "Context Rot," NoLiMa, RULER, Lost in the Middle — all shows real effects starting tens of thousands to hundreds of thousands of tokens past where any skill here sits). The one real, Claude-specific cliff found ("Prompt Design at Scale," arXiv 2607.19257) is about stacked-instruction *count*, not token length, and a keyword-based rule-line counter was tried and dropped: the paper's 20/40-instruction collapse zone was measured against a synthetic one-instruction-per-line prompt, and a naive count against this repo's actual prose (which routinely packs 2-3 imperative words into one ordinary sentence) flagged all 26 of 26 skills — not a usable signal. Treat the token warning as a prompt to do a compaction pass — `/skill-forge` has guidance — not as a gate to route around.
