...
- **Keep skill size in view while composing — and hold atoms to a stricter budget than skills.** `make validate` runs `scripts/check-content-size.mjs`, which warns (never fails the build) on any `skills/*/SKILL.md` over a 50,000-token ballpark estimate (plain character count / 4, not a real tokenizer), and any `atoms/*/CONTENT.md` over **10% of that** — 5,000 tokens. The tighter atom budget isn't arbitrary: an atom is inlined verbatim into every skill that composes it, so its size is paid by each of them, not once. **The 50,000-token figure is a deliberate, un-derived common-sense ceiling (~5% of Claude's 1M-token context), not a research-backed number** — checked directly, and no source ties a specific token count to measured output quality at the sizes these files actually reach (the token-length-degradation research that exists — Chroma's "Context Rot," NoLiMa, RULER, Lost in the Middle — all shows real effects starting tens of thousands to hundreds of thousands of tokens past where any skill here sits). The one real, Claude-specific cliff found ("Prompt Design at Scale," arXiv 2607.19257) is about stacked-instruction *count*, not token length, and a keyword-based rule-line counter was tried and dropped as a companion check: the paper's 20/40-instruction collapse zone was measured against a synthetic one-instruction-per-line prompt, and a naive count against this repo's actual prose (which routinely packs 2-3 imperative words into one ordinary sentence) flagged all 26 of 26 skills — not a usable signal, and not worth reviving until a counting method is found that actually discriminates on this repo's prose style. Treat the token warning as a prompt to do a compaction pass — `/skill-forge` has guidance — not as a gate to route around.
...
`delegates N setup steps` currently appears across **12** docs pages at four different values of N — 9 pages at one, 1 at two, 1 at three, 1 at seven. Recount rather than trusting this line: it dropped from 24 to 10 in one pass when 13 skills atomized onto compile-time composition and lost their `## Composes onto` section entirely, rather than changing their count — a swing that size isn't a mistake, it's what an atom migration actually does to this number — it ticked up to 11 when `connect-accounting` split out of `close-books` as its own delegated step, and to 12 when `assign-missing-invoices`, itself atom-composed, gained a `## Composes onto` section of its own for the one runtime dependency no atom covers: `connect-bank`. Only the pages whose dependency count actually changed may be touched. A repo-wide substitution to fix five pages will silently make ten wrong, and nothing validates prose.
...
6. **Verify `dist/` yourself.** A commit touching `skills/<name>/SKILL.md` must *not* carry a rebuilt `dist/<name>.zip` or `dist/<name>.skill` — either one present is a finding on its own. `.github/workflows/rebuild-dist.yml` rebuilds every archive after the PR merges to `main` and opens its own PR with the result; see [`dist/README.md`](dist/README.md).
