...
- **Keep skill size in view while composing — and hold atoms to a stricter budget than skills.** `make validate` runs `scripts/check-content-size.mjs`, which warns (never fails the build) on any `skills/*/SKILL.md` over a ~8000-token ballpark estimate (plain character count / 4, not a real tokenizer), and any `atoms/*/CONTENT.md` over **10% of that** — 800 tokens. The tighter atom budget isn't arbitrary: an atom is inlined verbatim into every skill that composes it, so its size is paid by each of them, not once — a skill composing a few atoms at full skill-sized budgets each would blow its own budget before writing a line of its own logic. Treat either warning as a prompt to do a compaction pass — `/skill-forge` has guidance — not as a gate to route around. As of this writing, 4 of the 6 atoms (`define-period`, `define-workspace`, `confirm-my-company`, `connect-tools`) are over budget — a real backlog, not a hypothetical the check exists to prevent.
