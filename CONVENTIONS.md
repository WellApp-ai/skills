...
- **Keep skill size in view while composing.** A skill built from many inlined atoms can grow long. `make validate` runs `scripts/check-content-size.mjs`, which warns (never fails the build) on any `skills/*/SKILL.md` or `atoms/*/CONTENT.md` over a ~8000-token ballpark estimate (plain character count / 4, not a real tokenizer). Treat the warning as a prompt to do a compaction pass — `/skill-forge` has guidance — not as a gate to route around.
