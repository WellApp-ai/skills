...
It carries the order, not the steps. Ten of its beats are each their own Well skill (`define-workspace`, `define-period`, `connect-bank`, `categorize-counterparties`, `assign-missing-invoices`, `show-missing-invoices`, `connect-tools`, `deploy-agents`, `invite-teammates` and `connect-accounting`), which this file runs in a fixed order rather than repeating what they do, plus the own-company beat it hands to `confirm-my-company`, the bank connection held one level up that it brings across through `workspace-data-migration` rather than asking you to connect the same bank twice, and a close that ends every walk. Install it on its own and only the workspace step stops the flow: eight steps fall back to an inline copy of the part the order depends on, and the bank step falls back to `connect-tools` scoped to banks. The categorization step runs only when your Well server exposes the counterparty list; without it the flow says so instead of guessing. A caller that has already fixed the workspace, period and scope (the close-books flow) runs this in composed mode, starting at the owners.
...
[⬇ Install fetch-missing-invoices](https://github.com/WellApp-ai/skills/raw/main/dist/fetch-missing-invoices.skill) and open the downloaded file. Desktop installs the skill straight away, with nothing to unzip.
