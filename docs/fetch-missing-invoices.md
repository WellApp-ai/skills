...
It orchestrates the Well bricks — `define-workspace`, `connect-tools`, `connect-bank`, `define-period`, `show-missing-invoices`, and `deploy-agents` — and bundles them, so installing this one skill brings the whole flow. The counterparty-categorization step is used when that brick is installed alongside; without it the flow says so instead of guessing.
